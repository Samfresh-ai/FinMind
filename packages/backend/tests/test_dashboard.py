...
def test_multi_account_overview_aggregates_and_returns_per_account(client, auth_header):
    ...
    r = client.get("/dashboard/multi-account-overview", headers=auth_header)
    assert r.status_code == 200
    payload = r.get_json()

    assert payload["aggregated"]["monthly_income"] == 500.0
    assert payload["aggregated"]["monthly_expenses"] == 80.0
    assert payload["aggregated"]["net_flow"] == 420.0

    account_keys = sorted([a["account_key"] for a in payload["accounts"]])
    assert account_keys == ["INR", "USD"]
